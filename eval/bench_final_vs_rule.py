...
def run_bench(args: argparse.Namespace) -> dict:
    """One arena run of HarnessAgent (A) vs RuleAgent (B); returns the result dict."""
    ...
    report = run_arena(
        ...
        run_label=f"bench_final_vs_rule_n{args.n}_s{args.seed}",
    )
...
def aggregate(paths: list[str]) -> dict:
    ...
    return {
        ...
        "n": n,
        "final_wins": wins,
        "rule_wins": sum(c["rule_wins"] for c in chunks),
        ...
        "rule_faults": sum(c["rule_faults"] for c in chunks),
...
def finish(result: dict, json_path: str | None, n_requested: int) -> int:
    ...
    print(
        f"BENCH final(harness) vs rule: n={result['n']} "
        f"W/D/L(final)={result['final_wins']}/{result['draws']}/{result['rule_wins']} "
        f"undecided={result['undecided']} win_rate={result['final_win_rate']:.3f} "
        f"Wilson95=[{ci_low:.4f}, {ci_high:.4f}] "
        f"faults(final/rule)={result['final_faults']}/{result['rule_faults']}"
    )
    print(
